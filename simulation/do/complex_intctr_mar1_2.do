...
cap log close
log using complex_intctr_mar1_2, smcl replace
...
tempname postseed
postfile `postseed' int(rep) str2000(s1 s2) str1100(s3) using "complex_intctr_mar1_rng_2.dta", replace

tempname sim
postfile `sim' int(rep) str28(method) float(mcra mderiv b0 bx se0 sex df0 dfx errcode) using complex_intctr_mar1_2, replace
