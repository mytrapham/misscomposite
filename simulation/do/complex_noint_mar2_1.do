...
cap log close
log using complex_noint_mar2_1, smcl replace
...
tempname postseed
postfile `postseed' int(rep) str2000(s1 s2) str1100(s3) using "complex_noint_mar2_rng_1.dta", replace

tempname sim
postfile `sim' int(rep) str28(method) float(mcra mderiv b0 bx se0 sex df0 dfx errcode) using complex_noint_mar2_1, replace
...
qui	{
	...
	forval r = 1/`R' 	{
		...
		//Cases: 1-case I (noint); 2-case II (intctr); 3-case III (inttrtctr)
		//Missingness mechanisms: 1-MCAR; 2-MAR1; 3-MAR2
		compositesim, outcometype(2) case(1) missingness(3) n(`n') m(`m') burnin(`c')
