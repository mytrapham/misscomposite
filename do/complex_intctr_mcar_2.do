...
tempname postseed
postfile `postseed' str2000(s1 s2) str1100(s3) using "complex_intctr_mcar_rng_2.dta", replace

tempname sim
postfile `sim' int(rep) str28(method) float(mcra mderiv b0 btrt se0 setrt df0 dftrt errcode) using complex_intctr_mcar_2, replace
