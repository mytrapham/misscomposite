...
cap log close
log using simple_intctr_mcar_2, smcl replace
...
tempname sim
postfile `sim' int(rep) str28(method) float(mcra mderiv b0 btrt se0 setrt df0 dftrt) using simple_intctr_mcar_2, replace
...
qui	{
	...
	forval r = 1/`R' 	{
		...
		//Case: 1-case I (noint); 2-case II (intctr); 3-case III (inttrtctr)
		//Missingness: 1-MCAR; 2-MAR1; 3-MAR2
		compositesim, outcometype(1) case(2) missingness(1) n(2000) m(10) burnin(15)
