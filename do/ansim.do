...
*cd "\\ad.ucl.ac.uk\home9\zcakf79\DesktopSettings\Desktop\test\output"
...
foreach type in simple complex {
	foreach case in noint intctr inttrtctr	{
		...
		foreach miss in mcar mar1 mar2	{
			...
			recode meth (3=1) (1=2) (2=3) (7=4) (8=5) (6=6) (4=7) (5=8)

			lab drop meth
			lab def meth 1 "Full data" 2 "CRA" 3 "Deriv" 4 "MI-CRA" 5 "MI-Deriv" ///
						 6 "MI component main" 7 "MI component by trt" 8 "MI component by trt z{subscript:1}" 
			lab val meth meth

			drop method
			compress			 
					
			foreach param in 0 trt 	{   
		...
		*Save results into separate datasets for graphs
		...
		foreach param in 0 trt	{
			...
			* BIAS
			...
			lab def method 1 "Full data" 2 "CRA" 3 "Deriv" 4  "MI compn main" 5 "MI compn by trt" 6 "MI compn by trt z{subscript:1}" 
			lab val method method
			...
			sort mismech method param
				...
				twoway (scatter method b if method == 1 & param == `p', msym(o) msiz(large) mcol(mrcgrey)) 					///
					   ...
					   (rspike ub lb method if method == 6 & param == `p', hor lcol(mrcyellow)) 							///
						...
						legend(order(1 "Full data" 3 "CRA" 5 "Deriv" 7 "MI compn main" 9 "MI compn by trt" 11 "MI compn by trt z{subscript:1}") ///
						col(1) size(vsmall))
